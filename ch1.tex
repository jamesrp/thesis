...
Because we can cancel squarefree terms mod $I$, every function in $\RR[C]$ has a unique represenative of lowest degree.
...
Recall from Section \ref{section{cosos} that we have a notion of sum of squares mod $I$.
...
In general it is thought that the degrees of these multipliers must be large, but no explicit bounds have been given so far.
We find a simple degree-4 polynomial in $n$ variables which requires multipliers of degree at least $n/2$.

We also reprove a result of Laurent on the max cut polytope's theta heirarchy.
We show that the theta body does not converge until at least the $n/2$ level.



\subsection{Background}
laurent result on lasserre rank of max cut = n/2

previous results on global sos multipliers
...
We abuse notation here and say {\em matching} for perfect matching; normally, a matching is a perfect matching with the vertex condition removed.
Let $M_{2n}$ be the set of all matchings in the complete graph $K_{2n}$.
The symmetric group $S_{2n}$ acts naturally on $M_{2n}$.
...
\subsection{Results}
\begin{itemize}
\item We find an explicit isomorphism between $\CC[M_{2n}]$ and $\oplus_\lambda S^\lambda$, where the sum is over all partitions $\lambda \vdash 2n$ having only even parts.
To construct the isomorphism, we give a map from each $S^\lambda$ to $\CC[M_{2n}]$ by imposing a matching structure on the group algebra elements in $S^\lambda$.
...
The decomposition here originally arose due to work on Chapter \ref{chap:cube}.
It is possible to apply the symmetry techniques used there to varieties beyond the hypercube.
In general, since the decompositions of these varieties into $S_n$ irreducibles are more complicated than that of the hypercube, it is harder to prove general statements.
However, we were able to use the symmetry reduction to compute for the matchings on $K_9$ and show that the second theta body did not capture the convex hull in this case, a previously open question.
